#!/usr/bin/env python3
"""Backtest the original hold-until-profitable strategy with a 1% net target.

Rules:
- BTC/USD, Bitstamp one-minute OHLC data.
- $300 initial cash.
- Allocate $1 at each eligible minute close.
- Each purchase is a separate lot.
- Reinvest proceeds immediately.
- Never force-sell an unprofitable lot.
- Sell an older lot when the subsequent candle high reaches the reference-price
  level needed to deliver exactly 1.00% net profit after modeled costs.
- Fee: 0.10% per side. Slippage: 0.01% per side.
...
# If the observed reference close at entry is P, this is the observed reference
# high needed later so that the $1 all-in allocation becomes $1.01 after the
# buy fee/slippage and sell fee/slippage.
TARGET_FACTOR = (
    (1.0 + NET_PROFIT_RATE)
    * (1.0 + FEE)
    * (1.0 + SLIPPAGE)
...
# Liquidation value of one $1 all-in lot at a later observed reference price Q:
# Q * EXIT_VALUE_FACTOR / entry_reference_price.
EXIT_VALUE_FACTOR = (
    BUY_ALLOCATION
    * (1.0 - FEE)
    * (1.0 - SLIPPAGE)
...
class Strategy:
    def __init__(self, name: str, monthly_withdraw_fraction: float) -> None:
        ...
        # Heap item: (target reference price, serial, entry reference close, entry ts)
    ...
    def process_minute(self, ts: int, high: float, close: float) -> None:
        # Lots inserted at the end of a minute are not eligible until the next
        # minute, avoiding same-candle look-ahead ambiguity.
    ...
    def close_month(self, month_label: str, ts: int, close: float) -> None:
        # Withdrawal is based on realized take-profit income, but may occur only
        # when cash exists and book capital remains at least the initial $300.
    ...
    def result(self, final_ts: int, final_price: float) -> dict[str, Any]:
        ...
        return {
            ...
            "buys": self.buys,
            "sales": sales,
            "participation_percent_of_minutes": None,
            "skipped_buys": self.skipped_buys,
            "realized_profit_before_final_liquidation": (
                sales * BUY_ALLOCATION * NET_PROFIT_RATE
            ),
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit(
            "Usage: btc_minute_backtest_1pct.py /path/to/btcusd.csv.gz"
        )
    ...
    strategies = [
        Strategy("full_reinvestment_1pct", 0.0),
        Strategy("withdraw_50_percent_of_monthly_realized_profit_1pct", 0.5),
        Strategy("withdraw_100_percent_of_monthly_realized_profit_1pct", 1.0),
    ]
    ...
    with gzip.open(data_path, "rt", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not (
                ...
                raise ValueError(
                    f"Invalid price at {ts}: high={high}, close={close}"
                )
    ...
    final_date_label = datetime.fromtimestamp(
        final_ts, timezone.utc
    ).date().isoformat()
    for strategy in strategies:
    ...
    for strategy in strategies:
        # Fill this only after rows_used is known.
        pass
    ...
    for strategy in strategies:
        item = strategy.result(final_ts, final_price)
        item["participation_percent_of_minutes"] = (
            100.0 * strategy.buys / rows_used
        )
    ...
    output = {
        "test": {
            ...
            "target_net_profit_percent_per_closed_lot": (
                NET_PROFIT_RATE * 100.0
            ),
            "required_reference_price_rise_percent": (
                TARGET_FACTOR - 1.0
            ) * 100.0,
            "execution": (
                "sell eligible older lots using candle high; then buy at candle close"
            ),
            "forced_exit": "none",
            "end_treatment": (
                "mark all open lots to modeled final liquidation value"
            ),
        },
        ...
        "buy_and_hold": {
            ...
            "return_percent": (
                buy_and_hold_final / INITIAL_CAPITAL - 1.0
            ) * 100.0,
        },
    }

    output_dir = Path("chatgpt_backtest/results_1pct")
    ...
    for strategy in strategies:
        write_csv(
            output_dir / f"monthly_{strategy.name}.csv",
            strategy.monthly_rows,
        )
        write_csv(
            output_dir / f"daily_{strategy.name}.csv",
            strategy.daily_rows,
        )
